  ...
  <div>
    <div class="historicalArcBox">
      ...
      <div class="historicalTable">
        <div v-for="( itemes, index) in tableData" :key="itemes.id" class="itemes">
          <div class="itemTopLab" :class="{act: isShow}">
            <div class="lab" @click="openTable(itemes.yearsMonth)"> &gt; </div>
            <div>
              <p class="title">{{ itemes.yearsMonth }}社保报表 <span>{{ itemes.creationTime }}</span></p>
              <a class="labTit" @click="openTable(itemes.yearsMonth)">点击查看</a>
            </div>
            <div>
          ...
          <!-- 报表 -->
          <div v-show="isShow" class="itemDropDown">
            <div class="topLab">
...
export default {
  ...
  mounted() {
    this.getArchivingListInfo()
  },
  methods: {
    // 初始化历史归档列表请求
    async getArchivingListInfo() {
      const res = await getArchivingList(this.yearVal)
      this.tableData = res
    },
    // 日期处理函数
    changeYear(date) {
    ...
    // 点击展开列表
    async openTable(yearsMonth) {
      // 报表请求数据
      try {
        const res = await getArchivingCont({ month: yearsMonth, year: this.yearVal, opType: 2 })
    ...
    // 入职时间格式化
    transformDateFormat(row, column) {
