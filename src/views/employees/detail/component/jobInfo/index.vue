<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="UpdatePostMag">保存更新</el-button>
...
import { getPostMag, getEmployeeSimple,UpdatePostMag} from "@/api/employees";
export default {
  name: "JobInfo",
  data() {
    return {
  ...
  created() {
    this.getPostMag() // 获取员工岗位信息
    this.getEmployeeSimple()  //  获取负责人简单列表
  },
  methods: {
    // 获取员工岗位信息
    async getPostMag() {
      try {
        this.formData = await getPostMag(this.userId);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async UpdatePostMag(){
      try{
        const data = {...this.formData,userId:this.userId}
        await UpdatePostMag(data)
