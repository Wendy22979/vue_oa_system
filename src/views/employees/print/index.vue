...
<script>
import { GetPersonalDetails, getPostMag } from "@/api/employees";
...
export default {
  ...
  methods: {
    ...
    async getJobDetail() {
      const userInfo = await getUserDetailById(this.userId); //获取用户头像
      const jobInfo = await getPostMag(this.userId); // 获取个人岗位信息
